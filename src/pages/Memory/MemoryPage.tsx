...
function PhotoCarousel({ photos, frameStyle }: { photos: string[]; frameStyle: string }) {
  ...
  const wrapperStyle: React.CSSProperties =
    frame === "polaroid" ? { background: "#fff", borderRadius: 16, padding: "12px 12px 40px", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" } :
    frame === "vintage" ? { background: "#f5e6c8", borderRadius: 8, padding: "10px 10px 36px", boxShadow: "0 4px 20px rgba(0,0,0,0.5)", filter: "sepia(0.3)" } :
    frame === "minimalista" ? { borderRadius: 16, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.3)" } :
    frame === "neon" ? { borderRadius: 16, padding: 3, background: "linear-gradient(135deg, #7c6aff, #b06fff, #ff6aff)", boxShadow: "0 0 20px rgba(124,106,255,0.6), 0 0 40px rgba(176,111,255,0.3)" } :
    { background: "#fff", borderRadius: 16, padding: "12px 12px 40px", boxShadow: "0 8px 32px rgba(0,0,0,0.4)" };

  const innerStyle: React.CSSProperties =
    frame === "neon" ? { borderRadius: 14, overflow: "hidden" } :
    frame === "minimalista" ? {} :
  ...
  return (
    <>
      ...
      <div style={frame === "neon" ? { ...wrapperStyle, animation: "neon-glow 2s ease-in-out infinite" } : wrapperStyle}>
        <div style={innerStyle}>
          <div className="relative w-full overflow-hidden" style={frame === "neon" || frame === "minimalista" ? { borderRadius: frame === "neon" ? 14 : 16 } : {}}>
              ...
              <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                {photos.map((_, i) => (
                  <button key={i} onClick={() => setIdx(i)}
                    ...
                    style={{ width: i === idx ? 20 : 6, background: i === idx ? "rgba(255,106,255,1)" : "rgba(124,106,255,0.9)" }}
                  />
                ))}
              </div>
            )}
...
function EmojiRain({ emoji }: { emoji: string }) {
  const drops = useRef(
    Array.from({ length: 18 }, (_, i) => ({
      ...
      duration: 4 + Math.random() * 4, size: 16 + Math.random() * 20,
    }))
  ).current;
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 1 }}>
        {drops.map((drop) => (
          <div key={drop.id} className="absolute"
            style={{ left: `${drop.x}%`, top: -40, fontSize: drop.size, animation: `emojifall ${drop.duration}s ${drop.delay}s linear infinite` }}>
            {emoji}
          </div>
        ))}
      </div>
...
function EntryButton({ emoji, bgColor, onReveal }: { emoji: string; bgColor: string; onReveal: () => void }) {
  ...
  const particles = useRef(
    Array.from({ length: 24 }, (_, i) => ({ id: i, angle: (i / 24) * 360, dist: 60 + Math.random() * 80 }))
  ).current;
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 flex items-center justify-center"
        style={{ zIndex: 50, background: `${bgColor}ee`, backdropFilter: "blur(8px)" }}>
        {exploding && particles.map((p) => {
          ...
          return (
            <div key={p.id} className="absolute"
              style={{ left: "50%", top: "50%", fontSize: 20, "--dx": `${dx}px`, "--dy": `${dy}px`, animation: "particle-fly 0.7s ease-out forwards" } as React.CSSProperties}>
              {emoji}
            </div>
          );
        })}
        <div style={{ perspective: "600px" }}>
          <button onClick={handleClick}
            ...
            style={{
              ...
              animation: exploding ? "none" : "btn-pulse 2s ease-in-out infinite, btn-float 3s ease-in-out infinite",
              transform: "rotateX(10deg)", transformStyle: "preserve-3d",
            }}>
            <div className="absolute inset-0 rounded-2xl"
              style={{ background: "linear-gradient(145deg, #4a2fc0, #3a1fa0)", transform: "translateZ(-8px) translateY(8px)", zIndex: -1 }} />
        ...
        <p className="absolute bottom-12 text-[12px] tracking-widest uppercase" style={{ color: "rgba(240,238,248,0.3)" }}>
          Abrir memória
        </p>
      </div>
    </>
...
export default function MemoryPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#09090f] flex flex-col items-center justify-center gap-3">
        <div className="w-8 h-8 rounded-full border-2 border-transparent animate-spin" style={{ borderTopColor: "#7c6aff", borderRightColor: "#b06fff" }} />
  ...
  const formattedDate = new Date(memory.date + "T12:00:00").toLocaleDateString("pt-BR", { day: "2-digit", month: "long", year: "numeric" });
  ...
  const textFont = memory.textFont === "serifada" ? "Georgia, serif" : memory.textFont === "manuscrita" ? "Caveat, cursive" : "DM Sans, sans-serif";

  return (
    <div className="min-h-screen text-white" style={{ background: memory.bgColor, fontFamily: "sans-serif", position: "relative" }}>

      <BgAnimation type={memory.bgAnimation} />
      {!revealed && <EntryButton emoji={emoji} bgColor={memory.bgColor} onReveal={() => setRevealed(true)} />}
      {revealed && <EmojiRain emoji={emoji} />}

      {revealed && (
        <div className="relative" style={{ zIndex: 2 }}>

          {/* Spotify — largura total no topo */}
          {memory.spotifyUrl && (
          ...
          {/* Layout side by side no desktop */}
          <div className="max-w-4xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex-1 flex flex-col gap-0  w-full lg:w-80 lg:flex-shrink-0">
              <div
                className="rounded-2xl p-6"
                style={{ background: "rgba(0,0,0,0.25)", backdropFilter: "blur(12px)", border: "1px solid rgba(255,255,255,0.07)" }}
              >
                {/* Contador */}
                <div className="rounded-xl px-4 py-3 text-center text-[13px] font-semibold tracking-wide mb-6"
                ...
                <div className="flex  items-start gap-2.5 mb-3">
                  <div className="w-0.75 self-stretch rounded-sm flex-shrink-0 bg-red-600" />
                  <h1 className="wrap-break-word  w-full p-1 text-[clamp(22px,4vw,36px)] font-extrabold uppercase tracking-widest text-white leading-tight  m-0"
                    style={{ fontFamily: titleFont, textShadow: "0 2px 12px rgba(0,0,0,0.5)" }}>
                    {memory.title}
                  </h1>
                </div>

                {/* Data */}
                <p className="text-[11px] tracking-widest uppercase mb-5 ml-5" style={{ color: "rgba(240,238,248,0.35)" }}>
                  {formattedDate}
                </p>
                ...
                {/* Texto */}
                <p className="text-[15px] leading-relaxed whitespace-pre-wrap mb-8"
                  style={{
