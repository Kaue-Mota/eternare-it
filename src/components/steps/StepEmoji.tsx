  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        ...
        <h2 className="text-[19px] font-medium text-white">Escolha o emoji</h2>
        <p className="text-[12px] text-white/50 mt-1">
          Ele vai chover pela página quando alguém abrir sua memória
        </p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        {EMOJI_OPTIONS.map((option) => {
          const isSelected = data.emoji === option.emoji
          return (
            <button
              key={option.emoji}
              type="button"
      ...
      {data.emoji && (
        ...
        >
          ...
          <p className="text-[12px]" style={{ color: 'rgba(240,238,248,0.5)' }}>
            Sua memória vai receber uma chuva de <strong className="text-white/70">{EMOJI_OPTIONS.find(e => e.emoji === data.emoji)?.label.toLowerCase()}</strong>
          </p>
        </div>
      )}
