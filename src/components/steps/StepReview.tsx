...
import { type MemoryFormData, BG_COLORS, type StepId } from '../../types/memory'
...
export function StepReview({ data, onEdit, onBack }: Props) {
  ...
  const colorLabel = BG_COLORS.find((c) => c.hex === data.bgColor)?.label ?? '—'

  const rows: { label: string; value: React.ReactNode; step: StepId }[] = [
    { label: 'Fotos', value: `${data.photos.filter(Boolean).length} foto(s)`, step: 1 },
    ...
    { label: 'Texto', value: data.text ? data.text.slice(0, 80) + (data.text.length > 80 ? '…' : '') : '—', step: 3 },
    {
      label: 'Cor de fundo',
      value: (
        <span className="flex items-center gap-2 justify-end">
          <span className="w-3.5 h-3.5 rounded-full border border-white/20 inline-block" style={{ background: data.bgColor }} />
          {colorLabel}
        </span>
      ),
      step: 4,
    },
    { label: 'Música', value: data.spotifyUrl ? 'Adicionada' : 'Não adicionada', step: 5 },
  ]
  ...
  async function handleCheckout() {
    ...
    try {
      ...
      data.photos.filter(Boolean).forEach((file) => formData.append('photos', file))
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <p className="text-[10px] tracking-[0.14em] uppercase text-[#b06fff] mb-1">Revisão final</p>
        <h2 className="text-[19px] font-medium text-white">Tudo certo?</h2>
        <p className="text-[12px] text-white/50 mt-1">Confira os dados antes de finalizar</p>
      </div>
        ...
        {rows.map(({ label, value, step }, i) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 min-w-0">
              <span className="text-[12px] text-white/80 text-right truncate">{value}</span>
              <button
                type="button"
                onClick={() => onEdit(step)}
      ...
      <div className="rounded-xl border border-[#7c6aff]/20 bg-[#7c6aff]/[0.06] px-4 py-4 flex items-center justify-between">
        <div>
          <p className="text-[13px] font-medium text-white">Memória eternizada</p>
          <p className="text-[11px] text-white/40 mt-0.5">Página pública + link permanente + QR Code</p>
        </div>
