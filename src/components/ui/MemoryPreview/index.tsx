...
export function MemoryPreview({ data }: Props) {
  ...
  useEffect(() => {
    ...
    if (photos.length < 2) { setSlideIdx(0); return }
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      {/* Moldura do celular */}
      <div
        className="w-55 rounded-[34px] border-2 border-white/10 overflow-hidden flex flex-col relative"
        style={{
          background: bgColor,
      ...
      >
        {/* Notch */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-17 h-4 bg-black/80 rounded-b-xl z-10 border-b border-white/5" />
          ...
          <div className="pt-5 pb-2 px-3 flex flex-col gap-1 bg-black/35">
            <div className="flex items-center gap-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[9px] text-white/40">Spotify • tocando agora</p>
              </div>
              <div className="w-5 h-5 rounded-full border border-white/20 flex items-center justify-center shrink-0">
                <svg width="6" height="6" viewBox="0 0 10 10" fill="rgba(255,255,255,0.7)">
        ...
        {/* Carrossel de fotos */}
        <div className="relative w-full h-49 overflow-hidden shrink-0">
          {photos.length > 0 ? (
            <>
              <div
        ...
        {/* Dots do carrossel */}
        <div className="flex justify-center gap-1.25 py-2" style={{ background: bgColor }}>
          {(photos.length > 0 ? photos : [null]).map((_, i) => (
            <button
              ...
              style={{
                ...
                background: i === slideIdx ? '#b06fff' : 'rgba(255,255,255,0.22)',
              }}
            />
          ))}
        </div>
        ...
        {/* Corpo da memória */}
        <div className="flex flex-col gap-2 px-3 pb-4" style={{ background: bgColor }}>
          {/* Contador */}
          ...
          <div className="flex items-start gap-2">
            ...
            {title ? (
              <p className="text-[15px] font-bold uppercase tracking-wider text-white leading-tight">
                {title}
              </p>
            ) : (
              <p className="text-[11px] text-white/30 italic">Seu título aqui</p>
            )}
          </div>

          {/* Texto */}
          {text ? (
            <p className="text-[11px] text-white/75 leading-relaxed">{text}</p>
          ) : (
...
function SpotifyProgressBar() {
  ...
  return (
    <div className="h-[2px] rounded-full bg-white/10 mx-1">
      <div
